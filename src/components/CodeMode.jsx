...
export default function CodeMode() {
  ...
  const ready = useModuleReady('code')
  const topicId = searchParams.get('topic') || CODE_TOPICS[0]?.id
  const topic = CODE_TOPICS.find(t => t.id === topicId) || CODE_TOPICS[0]
  const codeData = topic ? getCodeData(topic.id) : { questions: [] }
  ...
  const toggleNailed = (q) => {
    const id = qid(q)
    codeMastered.has(id) ? onUnnailCode(id) : onNailCode(id)
  }
  ...
  useEffect(() => {
    ...
    return focusScroll(() => document.getElementById('code-q-' + focusQ))
  }, [focusQ, topicId])
  ...
  return (
    <div className="written-page anim-fade">
      <div className="written-topbar">
        <button className="back-btn" onClick={() => backTo ? navigate(backTo) : navigate('/', { state: { module: 'code' } })}>
      ...
      <CategorySidebar
        ...
        onSelect={(t) => navigate('/code?topic=' + t.id)}
      />
      ...
      {visibleQuestions.length === 0 ? (
        ...
        <>
          <p className="section-label" style={{ marginBottom: 16 }}>
            {visibleQuestions.length} টি program
          </p>
          <div className="written-list">
            {visibleQuestions.map((q, idx) => (
              <CodeCard
                ...
                isNailed={codeMastered?.has(qid(q))}
...
function CodeCard({ domId, q, idx, topicColor, isOpen, isImportant, isNailed, onToggle, onToggleImportant, onToggleNailed }) {
  const a = q.answer
