...
export default function VivaNailedScreen() {
  ...
  const nailedByTopic = VIVA_TOPICS.map(t => {
    ...
    const items = (data.questions || [])
      .map(q => ({ q, qid: q._uid }))
      .filter(({ q, qid }) => vivaMastered.has(qid) && !trashedIds.has(q._id))
      ...
      {total === 0 ? (
        ...
        <>
          <div className="nailed-screen-summary">
            ...
            <span className="nailed-screen-total-label">viva question{total !== 1 ? 's' : ''} nailed across {nailedByTopic.length} topic{nailedByTopic.length !== 1 ? 's' : ''}</span>
          </div>
          ...
          <div className="nailed-screen-list">
            {nailedByTopic.map(({ topic: t, items }) => (
              <VivaNailedGroup key={t.id} topic={t} items={items} onUnnail={onUnnail} />
            ))}
          </div>
        </>
      )}
...
function VivaNailedGroup({ topic: t, items, onUnnail }) {
    ...
    <div className="nailed-group" style={{ '--c': t.color }}>
      ...
      {open && (
        <div className="nailed-group-body anim-slide" style={{ padding: '8px 6px 10px', gap: 10 }}>
          {items.map(({ q, qid }) => (
            <VivaNailedCard key={qid} q={q} qid={qid} topicColor={t.color} onUnnail={onUnnail} />
          ))}
        </div>
      )}
    </div>
...
function VivaNailedCard({ q, qid, topicColor, onUnnail }) {
  return (
