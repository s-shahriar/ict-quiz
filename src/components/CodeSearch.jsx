...
// Search across all Code programs. Item = { id, q, tags, answer:{ code, summary[], ... } }.
export default function CodeSearch({ onActiveChange, initialQuery = '' }) {
  ...
  const results = useMemo(() => {
    ...
    for (const t of CODE_TOPICS) {
      const questions = getCodeData(t.id)?.questions || []
      for (const q of questions) {
        const key = normalize(q.q)
        if (seen.has(key)) continue
        // Match the problem statement and its tags — not the program body.
    ...
    <div className="group-search">
      <div className="gs-input-wrap">
        ...
        <input
          ...
          placeholder="Search all Java programs…"
          aria-label="Search code programs"
        />
        {query && (
              ...
              <div className="gs-list">
                {pageItems.map(({ q, topic }) => (
                  <CodeResultCard
                    ...
                    topic={topic}
                    onOpen={() => {
                      const home = '/?module=code&search=' + encodeURIComponent(query)
                      navigate(home, { replace: true })
                      navigate('/code?topic=' + topic.id + '&q=' + q.id, { state: { backTo: home } })
                    }}
                  />
                ))}
              </div>
...
function CodeResultCard({ q, topic, onOpen }) {
  const Icon = topic.icon
  const summary = q.answer?.summary || []
  return (
    <div className="gs-card" style={{ '--c': topic.color }}>
      <button className="gs-card-topic" onClick={onOpen} title={`Open ${topic.name}`}>
        {Icon && <Icon size={13} />}
      ...
      {summary.length > 0 && (
        <div className="gs-card-exp">
          <Code2 size={12} style={{ color: topic.color, flexShrink: 0, marginTop: 1 }} />
