...
export default function VivaImportantScreen() {
  ...
  for (const t of VIVA_TOPICS) {
    for (const q of getVivaData(t.id).questions || []) {
        ...
        <>
          ...
          {activeGroup && (
            <div className="nailed-screen-list anim-fade" style={{ gap: 10 }}>
              {pageItems.map(({ q, qid }) => (
                <VivaImportantCard key={qid} q={q} qid={qid} topicColor={activeGroup.topic.color} onUnmark={weakOnly ? onUnweak : onUnmark} weakOnly={weakOnly} />
              ))}
...
function VivaImportantCard({ q, qid, topicColor, onUnmark, weakOnly }) {
  return (
