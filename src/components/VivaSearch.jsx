...
// Search across all Viva Q&A. Each question is { id, q, tags, answer:{ summary[], points[], ... } }.
export default function VivaSearch({ onActiveChange, initialQuery = '' }) {
  ...
  const results = useMemo(() => {
    ...
    const out = []
    const seen = new Set()
    for (const t of VIVA_TOPICS) {
      const questions = getVivaData(t.id)?.questions || []
    ...
    <div className="group-search">
      <div className="gs-input-wrap">
        ...
        <input
          ...
          placeholder="Search all Viva Q&A…"
          aria-label="Search viva questions"
        />
        {query && (
        ...
        <div className="gs-results anim-fade">
          ...
          {results.length === 0 ? (
            <div className="gs-empty">
              ...
              <p>No viva questions match your search.</p>
            </div>
          ) : (
            <>
              <div className="gs-list">
                {pageItems.map(({ q, topic }) => (
                  <VivaResultCard
                    ...
                    topic={topic}
                    onOpen={() => {
                      const home = '/?module=viva&search=' + encodeURIComponent(query)
                      navigate(home, { replace: true })
                      navigate('/viva?topic=' + topic.id + '&q=' + q.id, { state: { backTo: home } })
                    }}
                  />
                ))}
              </div>
...
function VivaResultCard({ q, topic, onOpen }) {
  const Icon = topic.icon
  const summary = q.answer?.summary || []
  return (
    <div className="gs-card" style={{ '--c': topic.color }}>
      <button className="gs-card-topic" onClick={onOpen} title={`Open ${topic.name} viva Q&A`}>
        {Icon && <Icon size={13} />}
      ...
      {summary.length > 0 && (
        <div className="gs-card-exp">
          <Mic size={12} style={{ color: topic.color, flexShrink: 0, marginTop: 1 }} />
