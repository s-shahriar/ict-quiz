...
export default function CodeNailedScreen() {
  const navigate = useNavigate()
  useModuleReady('code')
  const { value: codeMastered, remove: onUnnail } = useCodeMasteredContext()
  const { trashedIds } = useTrash()

  const nailedByTopic = CODE_TOPICS.map(t => {
    const data = getCodeData(t.id)
    const items = (data.questions || [])
      .map(q => ({ q, qid: q._uid }))
      .filter(({ q, qid }) => codeMastered.has(qid) && !trashedIds.has(q._id))
    ...
    <div className="nailed-screen nailed-screen--wide anim-fade">
      ...
      {total === 0 ? (
        <div className="nailed-screen-empty">
          ...
          <p>No programs nailed yet.</p>
          <span>Open any Code category and tap the <Star size={12} style={{ display: 'inline', verticalAlign: 'middle' }} /> icon on a card to mark it as nailed.</span>
        </div>
      ) : (
        <>
          <div className="nailed-screen-summary">
            <span className="nailed-screen-total">{total}</span>
            <span className="nailed-screen-total-label">program{total !== 1 ? 's' : ''} nailed across {nailedByTopic.length} categor{nailedByTopic.length !== 1 ? 'ies' : 'y'}</span>
          </div>
          ...
          <div className="nailed-screen-list">
            {nailedByTopic.map(({ topic: t, items }) => (
              <CodeNailedGroup key={t.id} topic={t} items={items} onUnnail={onUnnail} />
            ))}
          </div>
        </>
      )}
...
function CodeNailedGroup({ topic: t, items, onUnnail }) {
    ...
    <div className="nailed-group" style={{ '--c': t.color }}>
      ...
      {open && (
        <div className="nailed-group-body anim-slide" style={{ padding: '8px 6px 10px', gap: 10 }}>
          {pageItems.map(({ q, qid }) => (
            <CodeNailedCard key={qid} q={q} qid={qid} topicColor={t.color} onUnnail={onUnnail} />
          ))}
...
function CodeNailedCard({ q, qid, topicColor, onUnnail }) {
  return (
