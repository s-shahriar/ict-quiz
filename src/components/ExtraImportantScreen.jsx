...
export default function ExtraImportantScreen() {
  ...
  for (const t of EXTRA_TOPICS) {
    for (const q of getExtraData(t.id).questions || []) {
        ...
        <>
          ...
          {activeGroup && (
            <div className="nailed-screen-list anim-fade" style={{ gap: 10 }}>
              {pageItems.map(({ q, qid }) => (
                <ExtraImportantCard key={qid} q={q} qid={qid} topicColor={activeGroup.topic.color} onUnmark={weakOnly ? onUnweak : onUnmark} weakOnly={weakOnly} />
              ))}
