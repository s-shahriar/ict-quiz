// Supabase read/write for per-user progress (nailed / important / weak), keyed by the
...
// `batch` = [{ uid, patch: { nailed?, important?, weak? } }] where each patch holds only
...
// (e.g. nailed-only / important-only / important+weak) and send one upsert per group.
